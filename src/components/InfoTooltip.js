...
export default function InfoTooltip({ name, title, isOpend, onClose, handleCloseOverlay, loggedIn }) {
  return (
    <div className={`popup popup_name_${name} ${isOpend ? "popup_opend" : ""}`} onClick={handleCloseOverlay}>
      <div className={`popup__container popup__container_name_${name}`}>
        <button
          ...
          onClick={onClose}
        ></button>
        <img className="popup__img-tooltip" src={loggedIn ? rec1 : rec2} alt="крестик"/>
